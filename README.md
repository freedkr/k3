...
> **面试题目**：基于Mooncake论文实现分布式LLM推理系统的缓存管理模拟器

## 🎯 核心结论 - 简单策略的胜利

### 一句话总结
**在分布式LLM推理系统中，Random策略展现出96.6分的最佳通用性，证明了"范围优势 > 单点复用"的设计哲学。**

### 三大反直觉发现

#### 1️⃣ Random > CacheAware（极端热点场景）
```
🥇 Random + LFU:     28.58%  命中率，25.7% 集中度
🥉 CacheAware + LFU: 28.50%  命中率，100% 集中度（单点故障风险）
```
**原因**：CacheAware的贪心算法在极端热点下导致100%请求集中到单节点，而Random的天然分散反而更优。
...
### 第二阶段：算法实现与bug修复
**意外发现**：所有淘汰算法性能相同（都是31.8%）
**问题定位**：HashMap遍历导致算法退化为"随机淘汰"
**修复方案**：
- FIFO: 使用队列结构
- LRU: 双向链表 + HashMap
- LFU: 频率分组管理
...
**性能测试结果**：
```
Random + LFU:     28.58%（出乎意料的好）
CacheAware + LFU: 28.50%（理论最优却表现平平）
LFU > LRU（与论文预期相反）
```
...
3. ⚖️ 算法缺陷：hitCount权重过大，load权重形同虚设
